...
use editor_types::prelude::{Count, MoveDir1D, MovePosition};
...
fn the_shim_holds_the_cell_the_cursor_is_drawn_in_rather_than_the_character_it_sits_at() {
    ...
    assert_eq!(Count::Contextual, taken[0].count);
    assert_eq!(
...
fn replay(case: &Case, with_shim: bool) -> (Result<Outcome, Error>, Vec<(ScreenMotion, Count)>) {
    ...
    let taken = engine
        ...
        .map(|shim| {
            shim.intercepted()
                .iter()
                .map(|taken| (taken.motion, taken.count.clone()))
                .collect()
        })
        .unwrap_or_default();
