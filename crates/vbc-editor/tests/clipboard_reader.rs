...
//! asked once rather than in a loop, that two pastes racing each other cost one read between them,
//! that a read which lands after it was given up on stays out of the buffer, and that a failure
//! never falls back on what an earlier read found.
//!
...
//! the way the user experiences it: a stand-in render loop draws frames throughout, and the test is
//! red if the frames stop coming or if any one of them waits.
//!
...
/// The longest starting the reader may take, which is on the far side of [`STARTUP`] because
...
/// The pastes that race each other when the test is about two of them arriving at once.
...
impl Frames {
    ...
    /// Draws frames until the given moment, pasting what the read comes back with and timing every
    /// frame's look at it.
    fn draw_until(&mut self, request: &Request, until: Instant) {
        while Instant::now() < until {
            let asked = Instant::now();
            let progress = request.poll();
            ...
            if let Progress::Ready(outcome) = progress {
...
fn a_five_second_clipboard_keeps_the_frames_coming_and_pastes_nothing() -> Result<()> {
    ...
    let reader = reader_of(&stub);
    let request = reader.request();

    let mut frames = Frames::over(TYPED);
    frames.draw_until(&request, Instant::now() + SLOW_READ + PATIENCE);
...
fn a_refusing_clipboard_is_asked_once_and_not_again() -> Result<()> {
    ...
    let mut frames = Frames::over(TYPED);
    frames.draw_until(&request, Instant::now() + WATCH);
...
#[test]
fn two_pastes_at_once_cost_one_read_and_agree_on_it() -> Result<()> {
    let stub = holding_text(SOFT_DEADLINE * 2);
    let reader = reader_of(&stub);
    let gate = Barrier::new(REQUESTERS);

    let raced: Vec<thread::Result<Frames>> = thread::scope(|scope| {
        let requesters: Vec<_> = (0..REQUESTERS)
            .map(|_| {
                scope.spawn(|| {
                    ...
                    frames.draw_until(&request, Instant::now() + PATIENCE);
    ...
    for frames in raced {
        let frames = frames.map_err(|_| anyhow!("a paste panicked"))?;
        assert_eq!(
            frames.answers,
...
fn a_read_landing_after_the_hard_deadline_stays_out_of_the_buffer() -> Result<()> {
    ...
    let mut frames = Frames::over(TYPED);
    frames.draw_until(&request, Instant::now() + HARD_DEADLINE + FRAME_BOUND);
    assert_eq!(
        frames.answers,
        vec![Outcome::Unavailable(Reason::Abandoned)]
    );
    ...
    frames.draw_until(&request, Instant::now() + LATE_READ);
...
fn a_failure_pastes_nothing_rather_than_what_the_last_read_found() -> Result<()> {
    ...
    let mut frames = Frames::over("");
    frames.draw_until(&first, Instant::now() + PATIENCE);
    assert_eq!(frames.buffer, CLIPBOARD_TEXT);
    ...
    after.draw_until(&second, Instant::now() + PATIENCE);
